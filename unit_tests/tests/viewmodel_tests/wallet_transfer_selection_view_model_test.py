...
def test_on_ln_node_start_success_with_keyring_enabled(mock_bitcoin_config, mock_get_value, mock_logger, mock_get_keyring_status, mock_is_wallet_initialized, mock_run_in_thread, wallet_transfer_selection_view_model):
    ...
    with patch('src.views.components.toast.ToastManager.info') as mock_toast:
        with patch('src.data.repository.setting_repository.SettingRepository') as mock_setting_repo:
            with patch('src.data.repository.setting_repository.SettingRepository.get_wallet_network') as mock_get_network:
                # Configure mocks
                mock_is_wallet_initialized.return_value = Mock(is_wallet_initialized=True)
